...
sudo apt-get install libopenni2-dev libopenni-dev libopenni-sensor-primesense0 libopenni-sensor-primesense-dev libopenni-sensor-pointclouds0 libopenni-sensor-pointclouds-dev libusb-dev libusb-1.0-0-dev npm git
...
echo "PWD"
pwd
rosinstall_generator rosbash ros_tutorials log4cpp rosserial rosbridge_suite image_common tf2_web_republisher web_video_server image_pipeline depthcloud_encoder --rosdistro $ROSRELEASE --deps --wet-only --tar > tutorial.rosinstall
echo "PWD"
pwd
echo "LS"
ls -la ./*
...
echo "NEEDED_PKGS_DEPS: $NEEDED_PKGS_DEPS"
...
if [ ! -e $WORKSPACE/$PROJECT_NAME*.snap ]; then
    echo "failed to build $WORKSPACE/$PROJECT_NAME*.snap" >&2
    exit 1
fi
